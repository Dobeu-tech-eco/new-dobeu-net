...
## 2026-05-30 - Responsive Text Accessibility
**Learning:** When using Tailwind's `hidden` class (e.g., `hidden sm:inline`) to hide text responsively inside buttons or links, screen readers may lose context when the text is hidden on smaller screens.
**Action:** Always add an explicit `aria-label` and `title` to the parent element, and `aria-hidden="true"` to the child icon to ensure screen reader accessibility and provide a tooltip fallback.

...
## 2026-05-30 - Responsively Hidden Text in Buttons
**Learning:** Hiding text responsively (e.g., `hidden sm:inline`) in buttons leaves screen readers with only unlabelled icons on mobile devices.
**Action:** Always add an explicit `aria-label` and `title` to the parent element, and `aria-hidden="true"` to the child icon when text is responsively hidden to ensure proper accessibility.

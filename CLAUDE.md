...
`dobeu.net` v3 — Next.js 15 (App Router) marketing landing + client portal + admin panel for Dobeu Tech Solutions. TypeScript, Tailwind, shadcn/ui, Supabase backend, Stripe billing, deployed to Vercel.

`BRAINSTORM.md` and `PLAN.md` cover original scope and decisions. `STATUS.md` is the rolling launch log. `AGENTS.md` is the Cursor-cloud sibling of this file — keep the two in sync when commands change.
...
```bash
pnpm dev                       # dev server -> http://localhost:3000 (Turbopack OFF — see Conventions)
...
Embedded agent (server-side, admin-only):

```bash
pnpm add @composio/core @anthropic-ai/claude-agent-sdk   # required to activate lib/agent
pnpm tsx scripts/agent.ts "<prompt>"                     # standalone worker for long-running ops
```

CI (`.github/workflows/ci.yml`, self-hosted runner) runs `type-check + lint + build` on every push to `main` and every PR. `claude.yml` and `claude-code-review.yml` wire @claude mentions and auto-review.

## Testing layout

- Unit tests live alongside source: `lib/**/*.test.ts` **and** `app/api/**/route.test.ts`.
- `vitest.setup.ts` imports `@testing-library/jest-dom/vitest`; environment is `jsdom`; `include: ["**/*.{test,spec}.{ts,tsx}"]`; `e2e/**` excluded.
- E2E: `e2e/smoke.spec.ts` covers landing rendering, auth-gating redirects (`/portal` + `/admin` → `/login`), `/api/lead` happy + error paths, `robots.txt`/`sitemap.xml`, privacy/terms.
- Playwright (`playwright.config.ts`) spins up `pnpm dev` itself and reuses an existing one when present.
- Mock convention (Vitest 4.x): constructor mocks (e.g. `Resend`, the Stripe client) must use the `function` keyword inside `vi.fn(...)`, not arrow functions — also noted in `AGENTS.md`.

## Build verification

`next.config.ts` sets `typescript.ignoreBuildErrors: false` and `eslint.ignoreDuringBuilds: false` — **the build is a real verifier**. A green `pnpm build` means the code typechecks and lints. For a full pre-merge check, run `pnpm verify`.
...
- **Primary: `middleware.ts` → `lib/supabase/middleware.ts` (`updateSession`).** On every request it refreshes the Supabase session and redirects: `/portal/*` requires *any* authed user; `/admin/*` requires authed **and** email in `ADMIN_EMAILS` (env var, comma-sep).
...
- Env: copy `.env.example` → `.env.local`. `NEXT_PUBLIC_*` keys are client-exposed; everything else (`SUPABASE_SERVICE_ROLE_KEY`, `APOLLO_API_KEY`, `STRIPE_SECRET_KEY`, `STRIPE_WEBHOOK_SECRET`, `RESEND_API_KEY`, `UPSTASH_REDIS_REST_*`, `COMPOSIO_API_KEY`, `ANTHROPIC_API_KEY`, `CALENDLY_WEBHOOK_SIGNING_KEY`, etc.) is server-only.
- **`pnpm dev` deliberately does NOT use Turbopack.** See the comment in `components/analytics-provider.tsx` — Turbopack + Suspense streaming was unreliable for hydration in Next 15.5+ in dev.
